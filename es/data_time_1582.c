...
int main()
{
    uint16_t year; // variable declaration 
    uint8_t hour, min, sec, date, month;  // Again variable declaration
    ...
    LPC_RTC->DOM    = 4;   // Update date value 
    LPC_RTC->MONTH  = 4;   // Update month value
    LPC_RTC->YEAR   = 2020; // Update year value

    // Set Time 10:40:25 AM 
    LPC_RTC->HOUR   = 10;   // Update hour value 
    LPC_RTC->MIN    = 35;   // Update min value
